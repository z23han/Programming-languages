...
def removedead(fragment,returned): 
    ...
    test_fragment = fragment[::-1]
    test_returned = returned
    left_fragment = []
    for eqn in test_fragment:
        if eqn[0] in test_returned:
            left_fragment.append(eqn)
            test_returned.remove(eqn[0])
            for elt in eqn[1]:
                if elt not in test_returned:
                    test_returned.append(elt)
    return left_fragment[::-1]
'''            
# Solution version 2
def removedead(fragment, returned):
