...
def t_NUM(token):
    ...
    if token.value[0:2] != '0x':
        output_num = 0
        for number in token.value[:-1]:
            output_num += int(number)
            output_num *= 10
        output_num += int(token.value[-1])
    else:
        token.value = token.value[2:]
        output_num = 0
        for number in token.value[:-1]:
            if ord(number) >= ord('a'):
                number = ord(number) - ord('a') + 10
            else:
                number = int(number)
            output_num += number
            output_num *= 16
            print output_num
        if ord(token.value[-1]) >= ord('a'):
            output_num += ord(token.value[-1]) - ord('a') + 10
        else:
            output_num += int(token.value[-1])
            print output_num
    token.value = output_num
    token.type = "NUM"
    return token
...
question00 = "MY"
answer00 = [('ID', 'MY')]
print test_lexer(question00) == answer00
'''
